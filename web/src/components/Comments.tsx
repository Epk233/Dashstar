...
function MyComment({ comment, firstName, created_at, id, onDelete }: MyCommentProps) {
    const date = new Date(Number(created_at) * 1000);
    return (
        <>
            <Grid2 container sx={{ backgroundColor: "#f0f0f02d" }}>
                <Grid2 display="flex" sx={{ margin: 2 }}>
                    <Avatar sx={{ marginRight: 4, alignContent: "center" }}>{firstName?.slice(0, 4)}</Avatar>
                    <Box>
                        {comment ? (
                    ...
                    <Button onClick={() => onDelete(id)} sx={{ marginLeft: 'auto' }}>删除</Button>
                </Grid2>
            </Grid2>
        </>
    );
            ...
            <Card sx={{ marginTop: 3 }}>
                ...
                <Grid2>
                    {comments?.map((oneOfComments) => (
                        <MyComment
                            key={oneOfComments.id} // 使用唯一的评论ID作为key
